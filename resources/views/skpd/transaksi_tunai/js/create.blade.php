    ...
    $(document).ready(function() {
        ...
        $('#simpan_rekening').on('click', function() {
            ...
            if (kondisi.includes("4")) {
                alert('Tdk boleh memilih rekening lebih dari 1  dlm 1 no bku');
                return;
            }
        ...
        $('#simpan_transaksi').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('skpd.transaksi_tunai.cek_simpan') }}",
                type: "POST",
                dataType: 'json',
                data: {
                    no_bukti: no_bukti,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.message == '1') {
                        alert('Data berhasil ditambahkan, dengan Nomor Bukti : ' + response
                            .no_bukti);
                        window.location.href =
                            "{{ route('skpd.transaksi_tunai.index') }}";
                    } else {
